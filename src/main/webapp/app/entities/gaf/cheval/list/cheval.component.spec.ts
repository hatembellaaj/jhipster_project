import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { of } from 'rxjs';

import { ChevalService } from '../service/cheval.service';

import { ChevalComponent } from './cheval.component';

describe('Cheval Management Component', () => {
  let comp: ChevalComponent;
  let fixture: ComponentFixture<ChevalComponent>;
  let service: ChevalService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      declarations: [ChevalComponent],
    })
      .overrideTemplate(ChevalComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(ChevalComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(ChevalService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.chevals?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });
});
